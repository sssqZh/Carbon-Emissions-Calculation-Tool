...
// 碳排放计算引擎 — 纯函数模块，不依赖数据库
//
// 排放因子来源：
//   出行/饮食/消费 — 牛津大学 Our World in Data, 深圳市低碳公共出行方法学
//   居住能耗 — 生态环境部、国家统计局2023全国电力平均CO2排放因子, 建筑碳排放计算标准
//   发电碳足迹 — 生态环境部/统计局/能源局 2025年1月发布《2023年电力碳足迹因子数据》
//   食材碳足迹 — Poore & Nemecek (2018), 联合国气候变化及中国农业环境研究所
// ============================================================================
...
// 发电生命周期碳足迹 (kgCO2e/kWh) — 生态环境部2025年公告
...
const TRANSMISSION_FACTOR = 0.0463; // 电网输配电修正 (kgCO2e/kWh)

// 食材碳足迹 (kgCO2e/kg) — Poore & Nemecek (2018)
...
// ============================================================================
// Calculator: 个人碳足迹估算
// 公式: Carbon_total = Travel + Diet + Housing + Consumption
// ============================================================================

export function calculatePersonalFootprint(inputs: Record<string, unknown>): CalculationResult {
  const breakdown: BreakdownItem[] = [];

  // --- Travel ---
  let travelTotal = 0;
  const travelKeys: Array<{ key: string; factorKey: string }> = [
...
// 公式: EF_grid = Σ(pct_i / 100 × EF_i) + EF_transmission
// ============================================================================

export function calculateGridEmission(inputs: Record<string, unknown>): CalculationResult {
  const breakdown: BreakdownItem[] = [];
  let weightedTotal = 0;

  const powerKeys: Array<{ key: string; factorKey: string }> = [
  ...
  const total = weightedTotal + transmission;

  return {
    total_emission: +total.toFixed(6),
...
// ============================================================================
// Calculator: 火锅碳排放测算
// 公式: Carbon = Σ(portions_i × 0.2 kg × EF_food_i)
// ============================================================================

export function calculateHotpotEmission(inputs: Record<string, unknown>): CalculationResult {
  const breakdown: BreakdownItem[] = [];
  let total = 0;

  const foodKeys: Array<{ key: string; factorKey: string }> = [
