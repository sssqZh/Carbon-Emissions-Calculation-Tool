...
-- ============================================================================
-- Calculators (updated status to active)
-- ============================================================================
...
UPDATE calculators SET
  name = '个人碳足迹估算',
  description = '基于出行、饮食、居住和消费数据估算个人年度碳足迹。',
  category = '生活方式',
  status = 'active'
...
UPDATE calculators SET
  name = '当地电网平均碳排放因子计算',
  description = '根据本地发电结构占比估算电网平均碳排放因子。',
  category = '能源',
  status = 'active'
...
UPDATE calculators SET
  name = '火锅碳排放估算',
  description = '按火锅食材份数估算一次用餐的食材碳排放。',
  category = '场景活动',
  status = 'active'
...
INSERT INTO formula_versions (calculator_id, version, formula_text, assumptions, source_id, status)
SELECT id, '1.0',
  '总碳足迹 = 出行 + 饮食 + 居住 + 消费。'
  || ' 出行: Σ(各类交通工具里程 × 排放因子)。'
  || ' 饮食: 饮食天数 × 膳食类型日排放因子。'
  ...
  || ' 消费: Σ(购买数量 × 单品碳足迹)。',
  '按年度输入估算，结果单位为 kgCO2e。出行因子单位：私家车/电动车为 km 人均排放，公共交通为 passenger-km。',
  1, 'active'
...
SELECT id, '1.0',
  ...
  || ' 各发电类型因子来自生态环境部2023年电力碳足迹公告。',
  '各能源占比合计应约等于 100%。输配电修正为可选，系数 0.0463 kgCO2e/kWh。',
  4, 'active'
...
SELECT id, '1.0',
  ...
  || ' 食材因子来自 Poore & Nemecek (2018) 食品生命周期研究。'
  || ' 人均排放 = 总排放 ÷ 用餐人数。',
...
JOIN (
  ...
  -- grid-emission-factor: 10 fields
...
-- ============================================================================
-- Emission Factors (35 real records)
-- ============================================================================
